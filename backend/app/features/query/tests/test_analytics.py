...
async def test_another_tenants_activity_is_absent(account: Account) -> None:
    ...
    ours = await AnalyticsService(profile(account, account.admin_id, everyones=True)).overview()

    assert ours.totals.queries == 0
    assert all("their question" not in entry.question for entry in ours.recent)
...
async def test_the_audit_row_names_the_documents_the_answer_read(account: Account) -> None:
    ...
    assert recent[0].question == "who was paid what?"
    assert recent[0].documents == ["payroll.pdf"]
    assert not recent[0].abstained
