 ...
 *
 * The panel fetches passage and citation counts on mount, and that resolves after a
 * synchronous assertion returns — which React reports as an update outside `act`. The warning
 * is right: the assertions would be reading a half-rendered panel. Awaiting the flush is the
 * fix, not silencing it.
 */
const show = async (overrides: Record<string, unknown> = {}) => {
  await act(async () => {
    render(<DocumentDetail token="t" document={document_(overrides) as never} labelNames={[]} />);
  });
};

describe("a document that finished with something to report", () => {
  it("shows what the pipeline said about it", async () => {
    await show({ status_detail: "2 of 4 page(s) are not searchable (page 2, 4): no OCR" });
...
describe("a document that failed", () => {
  it("says so, and says why", async () => {
    await show({ status: "failed", status_detail: "no extractable text layer" });
